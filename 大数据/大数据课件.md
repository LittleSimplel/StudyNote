...
- 流程
  ...
  ![MapReduce流程图.png](http://ww1.sinaimg.cn/large/0062TeRXgy1gdw1llv4bzj30z60djn02.jpg)
...
- 优化 
  ...
  - 表连接优化：将大表放后头Hive假定查询中最后的一个表是大表。它会将其它表缓存起来，然后扫描最后那个表。因此通常需要将小表放前面

  - order by & sort byorder by : 对查询结果进行全局排序消耗时间长,需要set hive.mapred.mode=nostrictsort by : 局部排序，并非全局有序，提高效率。
